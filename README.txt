...
successfully download and run SPHEREFIT. A thorough explanation of the
source code, relevant variables and user settings is provided in
SPHEREFIT_guide.pdf, generated once SPHEREFIT.tgz is unarchived.
...
the bin/ directory. Go to the bin/ directory. Copy "default.txt" from
the data/ directory inside the bin/ directory. Do the same with the
file "spherefitsettings.txt" from the settings/ directory.
...
iii) Open spherefitsettings.txt. Make use that the "what_to_do" variable
...
several lines of commented-out lines (those that start with the
character "#"), that also appear on screen, the file lightcurve.txt
(or spectrum.txt) contains 4 columns. The first column denotes the data-
point number. The second and third columns carry the observer time
(in days) and frequency (in Hz) of a datapoint, respectively. The fourth
column contains the result of the calculation, which is the observed
flux, in units of mJy.
